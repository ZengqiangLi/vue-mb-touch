(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["vueMBTouch"] = factory();
	else
		root["vueMBTouch"] = factory();
...
// CONCATENATED MODULE: ./lib/VueTouch.ts


var supportsPassive;

try {
  var VueTouch_options = Object.defineProperty({}, "passive", {
    get: function get() {
      supportsPassive = true;
    }
  });
  window.addEventListener("test", null, VueTouch_options);
} catch (err) {
  supportsPassive = false;
}

var VueTouch_VueTouch =
/*#__PURE__*/
function () {
  function VueTouch(el, stop, prevent, passive, capture, proxy) {
  ...
  _createClass(VueTouch, [{
    ...
    value: function handleEvent(e) {
      ...
      switch (type) {
        ...
        case 'touchend':
          var fingers = this.getFingers(e);
          var distance = this.getDistance(this.spos, this.epos);
    ...
    value: function press(e) {
      ...
      this.pressTimeId = setTimeout(function () {
        var fingers = _this.getFingers(e);

        var distance = _this.getDistance(_this.spos, _this.epos);

        if (_this.vueTouchEvent.press && fingers === 1 && distance <= VueTouch.config.maxDistance) {
          _this.sendEvent(e, 'press');
        }
    ...
    key: "sendEvent",
    value: function sendEvent(e, name) {
...
var lib_install = function install(Vue, options) {
  if (options) {
    // 此处不适用es7 对象合并， 否则会使webpack编译出来的包变大
    if (options.maxDistance) {
      VueTouch_VueTouch.config.maxDistance = options.maxDistance;
    }

    if (options.pressTime) {
      VueTouch_VueTouch.config.maxDistance = options.pressTime;
    }
  }
  ...
  Vue.directive('touch', {
    bind: function bind(el, binding) {
      ...
      var passive = !!modifiers.passive;
      var capture = !!modifiers.capture;
      var proxy = !!modifiers.proxy;
      var tap = !!modifiers.tap;
      var press = !!modifiers.press;
      ...
      var touchElment = el;
      var vueTouch = new VueTouch_VueTouch(el, stop, prevent, passive, capture, proxy);
